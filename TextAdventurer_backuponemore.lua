...
local function GetDirectionToTarget()
  ...
  if not playerFacing then return nil end
  
  -- This requires the target to be on the same map
  ...
  if not playerPos or not targetPos then return "Target position unavailable." end
  
  local px, py = playerPos:GetXY()
  local tx, ty = targetPos:GetXY()
  local angleToTarget = math.atan2(ty - py, tx - px)
  local diff = math.deg(playerFacing - angleToTarget) % 360
  
  if diff < 22.5 or diff >= 337.5 then return "directly ahead"
  elseif diff < 67.5 then return "ahead and to your left"
  elseif diff < 112.5 then return "to your left"
  elseif diff < 157.5 then return "behind and to your left"
  elseif diff < 202.5 then return "behind you"
  elseif diff < 247.5 then return "behind and to your right"
  elseif diff < 292.5 then return "to your right"
  else return "ahead and to your right"
  end
end
...
local function ResetSwingTimer()
  ...
  AddLine("combat", "You ready your next strike.")
end
...
local function CheckSwingTimer()
  ...
  if remain <= 0 and TA.lastSwingState ~= "ready" then
    AddLine("combat", "Your next strike is ready.")
    TA.lastSwingState = "ready"
  elseif remain > 0 and remain <= 0.3 and TA.lastSwingState ~= "soon" then
    AddLine("combat", "Your strike is about to land again.")
...
local function DoTargetCommand(arg)
  ...
  elseif lower == "direction" then
    local dir = GetDirectionToTarget()
    if dir then
      AddLine("target", "Your target is " .. dir .. ".")
    else
      AddLine("system", "No target or unable to determine direction.")
    end
...
local function ProcessInputCommand(msg)
  ...
  elseif lower == "help" then
    AddLine("system", "Type status, where, xp, quests, buffs, tracking, inventory, gear, actions, spells, trainer, train <n>, train all, bind <slot> <spellbookIndex>, target nearest/next/corpse/<name>, marka, markb, spacing, ta input, explore, autoquests on/off, chat on/off, or clear.")
  else
    AddLine("system", "Unknown input. Try: status, where, xp, quests, buffs, tracking, inventory, gear, actions, spells, trainer, train 1, train all, bind 1 3, target nearest, marka, markb, spacing, ta input, explore")
  end
end
...
SlashCmdList.TEXTADVENTURER = function(msg)
  ...
  elseif lower == "help" then
    AddLine("system", "Commands: /ta show, /ta hide, /ta toggle, /ta clear, /ta textmode on, /ta textmode off, /ta autostart on, /ta autostart off, /ta status, /ta where, /ta xp, /ta quests, /ta buffs, /ta tracking, /ta inventory, /ta gear, /ta actions, /ta spells, /ta trainer, /ta train 1, /ta train all, /ta bind 1 3, /ta target nearest, /ta target next, /ta target corpse, /ta target <name>, /ta marka, /ta markb, /ta spacing, /ta input, /ta explore, /ta autoquests on, /ta autoquests off, /ta chat on, /ta chat off, /ta direction")
  else
